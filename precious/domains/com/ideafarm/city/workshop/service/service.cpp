...
{
    char* postLogDefault = "c:\\tmp\\ideafarm.service.log.txt" ;
        ...
        if( cba < 0x100 )
        {
            ...
            if( InitializeProcThreadAttributeList( (_PROC_THREAD_ATTRIBUTE_LIST*)pcInfo2 , 1 , 0 , &cba ) )
            {
                ...
                if( UpdateProcThreadAttribute( (_PROC_THREAD_ATTRIBUTE_LIST*)pcInfo2 , 0 , PROC_THREAD_ATTRIBUTE_PREFERRED_NODE , &offNodePreferred , sizeof offNodePreferred , 0 , 0 ) )
                {
                    ...
                    //CS:CODEsYNC: SERVICE.CPP SERVICE.CPP
                    //TO ELIMINATE THE KERNEL MONITOR CONSOLE WINDOW WHEN HIRING THE CONSOLE FROM THE STARTUP MENU, UNCONDITIONALLY SPECIFY DETACHED_PROCESS IN THE FOLLOWING CALL TO CreateProcess:

                    memset( &infoP.process , 0 , sizeof infoP.process ) ;
                    int rc = CreateProcess( 0 , posteP , 0 , 0 , 0 , ( /*U::bServiceP ?*/ DETACHED_PROCESS /*: 0*//*U::TO FIND A BUG: CREATE_NEW_CONSOLE*/ ) | EXTENDED_STARTUPINFO_PRESENT , 0 , posthP , (STARTUPINFO*)&info1 , &infoP.process ) ;
...
void hireF( homeDiskS& infoP , int bServiceP = 0 )
{
    ...
    strcat( poste , bServiceP ? "51000767" : "510004fb" ) ;             // IF SERVICE THEN LAUNCH CLOAK ; ELSE (IF CONSOLE) THEN LAUNCH KERNEL MONITOR, WHICH WILL BE TOLD TO HIRE CONSOLE
    strcat( poste , " !idHome " ) ;
    strcat( poste , postIdHomeless ) ;
    strcat( poste , bServiceP ? "" : " !console " ) ;
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    logTimeGF( "creating slab / return code from CreateFileMapping is aok ; calling MapViewOfFile for slab\r\n" ) ;
                    ...
                    else
                    {
                        ...
                        while( !bQuit && !cHired )
                        {
                            ...
                            logTimeGF( "serviceMainF/loop/napping\r\n" ) ;
...
int main( int cArgP , char* ppostArgP[] , char* ppostEnvP[] )
{
        ...
        else
        {
            ...
            if( !oshService )
            {
                ...
                switch( rc )
                {
                    ...
                    {
                        logTimeGF( "IPDOS (tm) Service / could not open the \"IPDOS (tm)\" service / ERROR_SERVICE_DOES_NOT_EXIST" ) ;
                        break ;
                    }
                    case ERROR_ACCESS_DENIED :
                    {
                        logTimeGF( "IPDOS (tm) Service / could not open the \"IPDOS (tm)\" service / ERROR_ACCESS_DENIED" ) ;
                        bHireConsole = 1 ; //ASSUME: I WAS LAUNCHED FROM THE START MENU
                        break ;
                    }
                    default :
                    {
            ...
            else
            {
                ...
                info.dwPreshutdownTimeout = 60000 ; // 1 FOREIGN MINUTES
    ...
    if( !bStop )
    {
        ...
        if( GetUserName( postUser , &costaUser ) )
        {
            ...
            //20230330@0704: ENABLE THIS TEMPORARILY FOR DEVELOPMENT
            ...
            //LAUNCH THE CONSOLE ADAM, SINCE I AM ASSUMING THAT I WAS HIRED FROM THE WINDOWS START MENU
            else
            {
                int cHired = 0 ;
